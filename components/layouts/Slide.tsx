        ...
        <div className="pt-[43px] relative">
            <div className="flex flex-row items-center justify-between mb-[5px]">
                ...
                <div className="flex flex-row">
                    <div
                        className={`w-[30px] h-[30px] bg-[#f2f2f2] group hover:bg-bg-main flex items-center justify-center rounded ${prevBtn}`}>
                        <GrFormPrevious className="text-[#333] group-hover:text-white-1 w-[16px] h-[16px]"/>
                    </div>
                    <div
                        className={`bg-[#f2f2f2] group hover:bg-bg-main w-[30px] h-[30px] flex items-center justify-center rounded ml-[5px] ${nextBtn}`}>
            ...
            <div className="py-[20px]">
                ...
                >
                    {Array.from({length: 10}).map((_, index) => (
                        <SwiperSlide key={`outer-${index - 1}`} className="grid grid-cols-3 gap-5">
                            {Array.from({length: 3}).map((_, subIndex) => (
                                <div className="mt-3" key={`inner-${index}-${subIndex + 1}`}>
                                    <SmallBoxProduct
